...
function hasRetinoid(p) {
  ...
  return /\b(retinal|retinol|tretinoin|adapalene|retinoic acid)\b/.test(ing);
}
...
function hasExfoliantAcid(p) {
  ...
  const ing = (p.ingredients || '').toLowerCase();
  return /\b(glycolic acid|lactic acid|mandelic acid|citric acid|salicylic acid|beta hydroxy|poly hydroxy|lactobionic|gluconolactone|capryloyl salicylic)\b/.test(ing);
}
...
// Priority: explicit image > explicit icon > category fallback > generic
function prodEmoji(p) {
  if (!p) return '🧴';
  if (p.image) return `<img src="${p.image}" alt="" class="prod-img-fallback" onerror="this.replaceWith(document.createTextNode('${getCategoryIcon(normalizedCategory(p))}'))">`;
  if (p.icon) return p.icon;
  return getCategoryIcon(normalizedCategory(p));
}
...
const T = {
  en:{
    ...
    morning_routine:'☀️ Daily Morning Base Routine',night_routine:'🌙 Night Routine',
    ...
    step_bha_note:'⚠️ Spot treat ONLY. Tiny amount per pimple.',
    step_sandwich_1_name:'Moisturizer — Sandwich Layer 1',
    step_sandwich_1_note:'Apply moisturizer FIRST before retinal.',
    step_retinal_note:'Eye area ONLY. Tiny amount.',
    step_sandwich_2_name:'Moisturizer — Sandwich Layer 2',
    step_sandwich_2_note:'Seal retinal with second moisturizer layer.',
    ...
    retinal_rule_body:'Eye area only · 2×/week max · Always SPF next morning · Sandwich between moisturizer',
  ...
  th:{
    ...
    morning_routine:'☀️ รูทีนเช้าพื้นฐาน',night_routine:'🌙 รูทีนกลางคืน',
    ...
    step_bha_note:'⚠️ แต้มเฉพาะหัวสิวเท่านั้น ปริมาณน้อย ๆ',
    step_sandwich_1_name:'มอยส์เจอไรเซอร์ — Sandwich ชั้นที่ 1',
    step_sandwich_1_note:'ทามอยส์เจอไรเซอร์ก่อน Retinal เสมอ',
    step_retinal_note:'เฉพาะรอบดวงตา ปริมาณน้อยมาก',
    step_sandwich_2_name:'มอยส์เจอไรเซอร์ — Sandwich ชั้นที่ 2',
    step_sandwich_2_note:'ปิดทับ Retinal ด้วยมอยส์เจอไรเซอร์อีกชั้น',
    ...
    retinal_rule_body:'เฉพาะรอบดวงตา · สัปดาห์ละไม่เกิน 2 ครั้ง · ตามด้วย SPF เช้าเสมอ · ประกบด้วยมอยส์เจอไรเซอร์',
...
const ACTIVE_LABELS = {
  ...
  'bha':'🔬 BHA','aha':'🔬 AHA','arbutin':'🌓 Arbutin','tranexamic acid':'🎨 Tranexamic Acid'
};
function prodActiveTags(p){
  return (p.activeIngredients||[]).map(a=>ACTIVE_LABELS[a]||`✦ ${a}`);
}
...
function renderLibrary(){
  ...
  if(countEl)countEl.textContent=`Showing ${filtered.length} of ${PRODUCT_DB.length} products`;
  if(!content)return;
  if(!filtered.length){content.innerHTML=`<div class="empty-lib"><div class="empty-lib-icon">🔍</div><div>No products match your search.</div></div>`;return;}
      ...
      <div class="product-row" data-product-id="${p.id}" role="button" tabindex="0" aria-label="View details for ${p.brand} ${p.name}">
        ...
        <div class="prod-info">
          ...
          <div class="prod-flags">
            ${p.fragranceFree?'<span class="prod-flag safe">Fragrance-Free</span>':'<span class="prod-flag danger">Has Fragrance</span>'}
            ${p.alcoholFree?'':'<span class="prod-flag warn">Contains Alcohol</span>'}
            ${p.eoFree?'':'<span class="prod-flag warn">Essential Oils</span>'}
        ...
        <div class="prod-ratings-sm">${ratingMini(r.sensitive,'Sensitive')}${ratingMini(r.barrier,'Barrier')}${r.aging>=3?ratingMini(r.aging,'Aging'):''}</div>
      </div>`;
    }).join('');
    return `<div class="brand-group"><div class="brand-header open" onclick="toggleBrand(this)"><span class="brand-name">${brand}</span><span class="brand-count">${prods.length} product${prods.length!==1?'s':''}</span><span class="brand-arrow">▼</span></div><div class="brand-products">${prodHTML}</div></div>`;
  }).join('');
...
function openProductModal(id){
  ...
  const warnings = [];
  if(!p.fragranceFree) warnings.push('⚠️ Contains fragrance/parfum — caution if fragrance-sensitive');
  if(!p.alcoholFree) warnings.push('⚠️ Contains drying alcohol — caution if dry or barrier-compromised');
  if(!p.eoFree) warnings.push('⚠️ Contains essential oils — caution if reactive');
    ...
    <div class="modal-hero">
      ...
      <div>
        ...
        <div class="modal-tag-row">
          ...
          ${p.fragranceFree?'<span class="modal-tag ff">Fragrance-Free</span>':'<span class="modal-tag noff">Has Fragrance</span>'}
          ${p.alcoholFree?'<span class="modal-tag ff">Alcohol-Free</span>':'<span class="modal-tag noff">Contains Alcohol</span>'}
          ${p.eoFree?'<span class="modal-tag ff">EO-Free</span>':'<span class="modal-tag noff">Essential Oils</span>'}
        </div>
      </div>
    </div>
    <div class="modal-body">
      ${description?`<div class="modal-sec"><div class="modal-sec-title">What It Does</div><div class="modal-text">${description}</div></div>`:''}
      
      ${actives.length?`<div class="modal-sec"><div class="modal-sec-title">Key Active Ingredients</div><div class="modal-key-ings">${actives.map(a=>`<span class="key-ing">${a}</span>`).join('')}</div></div>`:''}
      
      ${concerns.length?`<div class="modal-sec"><div class="modal-sec-title">Skin Concerns Addressed</div><div class="modal-key-ings">${concerns.map(c=>`<span class="key-ing">${c}</span>`).join('')}</div></div>`:''}
      
      <div class="modal-sec">
        <div class="modal-sec-title">Skin Suitability</div>
        <div style="display:flex;gap:18px;flex-wrap:wrap;padding:8px 0">
          ${ratingMini(r.sensitive,'Sensitive Skin')}
          ${ratingMini(r.barrier,'Barrier Repair')}
          ${ratingMini(r.aging,'Anti-Aging')}
        </div>
        <div style="display:flex;flex-direction:column;gap:6px;margin-top:8px;font-size:0.72rem;color:var(--ink2)">
          ${isSensitiveSafe?'<div>✅ Suitable for sensitive skin per brand claims</div>':''}
          ${isAcneSafe?'<div>✅ Suitable for acne-prone skin per brand claims</div>':''}
          ${isMatureSupport?'<div>✅ Supports mature skin / elasticity / fine lines</div>':''}
        </div>
      </div>
      
      ${warnings.length?`<div class="modal-sec"><div class="modal-sec-title">⚠️ Ingredient Warnings</div><div class="info-box amber">${warnings.join('<br>')}</div></div>`:''}
      
      <div class="modal-sec">
        <div class="modal-sec-title">How to Use</div>
        <div class="info-box green">
          <strong>Best for:</strong> ${bestFor}<br>
          <strong>How often:</strong> ${howOften}
        </div>
      </div>
      
      ${doNotCombine && doNotCombine.toLowerCase()!=='n/a' && doNotCombine.toLowerCase().indexOf('no significant')!==0 ? `
        <div class="modal-sec">
          <div class="modal-sec-title">Do Not Combine With</div>
          ${doNotChips.length 
        ...
        <div class="modal-sec">
          <div class="modal-sec-title">Routine Compatibility</div>
          <div class="info-box green">✅ No significant ingredient conflicts. Safe to layer with most other products.</div>
        </div>`
      }
      
      ${p.medicubeMode && p.medicubeMode!=='None' ? `
        <div class="modal-sec">
          <div class="modal-sec-title">💡 Medicube Booster Pro Compatibility</div>
          <div class="info-box green">
            <strong>Recommended Mode:</strong> ${p.medicubeMode}<br>
            ${p.medicubeMode==='Booster'?'💧 Use during Booster Mode to press water-based actives deeper into skin.':''}
            ${p.medicubeMode==='MC'?'🟣 Use during MC Mode for micro-current pairing. Phase 2+ only.':''}
            ${p.medicubeMode==='Derma Shot'?'🔴 Use during Derma Shot Mode for deeper penetration. Phase 2+ only.':''}
          </div>
        </div>` : `
        <div class="modal-sec">
          <div class="modal-sec-title">💡 Medicube Booster Pro</div>
          <div class="info-box amber">Not intended for device use. Apply manually only.</div>
        </div>`
      }
      
      <div class="modal-sec">
        <div class="modal-sec-title">Full INCI Ingredient List</div>
        <div class="full-ing">${p.ingredients?p.ingredients:'<em style="color:var(--ink2)">Ingredient data missing — verify ingredient list on your physical carton before use.</em>'}</div>
      </div>
      
      <div style="text-align:center;padding-top:8px"><button class="btn btn-rose" onclick="closeModal()">Close</button></div>
    </div>`;
...
function renderRoutineResult(c){
  ...
  const _hasActives=_selected.some(p=>(p.activeIngredients||[]).some(ai=>['retinal','aha','bha'].includes(ai)));
...
function renderRoutineResultBody(rd){
  ...
  const hasActives=selected.some(p=>(p.activeIngredients||[]).some(ai=>['retinal','aha','bha'].includes(ai)));
  ...
  const c1=selected.find(p=>p.subcategory==='cleansing balm')||byCategory('cleanser');
  const c2=selected.find(p=>p.category==='cleanser'&&p.subcategory!=='cleansing balm'&&p!==c1);
  const toner=byCategory('toner');
  const essence=byCategory('essence');
  const serum=byCategory('serum');
  const moist=byCategory('moisturizer');
  const moist2=byCategories(['moisturizer']).find(p=>p!==moist);
  const spf=byCategory('sunscreen')||selected.find(p=>isSunscreenProduct(p));
  const deviceGel=byCategory('device gel');
  const retinalProd=selected.find(p=>(p.activeIngredients||[]).includes('retinal'));
  const bha=selected.find(p=>p.subcategory==='spot treatment');
  const aha=selected.find(p=>(p.activeIngredients||[]).includes('aha'));
  const peel=selected.find(p=>p.category==='exfoliant');
  ...
  return `
    <div class="builder-card">
      ...
      ${renderMorningPhases(selected,toner,essence,serum,moist,spf,c1,c2,isHighSens)}
      ...
      ${renderPhase('p1',selected,c1,c2,toner,essence,serum,moist,moist2,deviceGel,usesDevice,false,false,false,false,isMature,isHighSens,'active',isBarrierHealthy)}
      ${numPhases>=2?renderPhase('p2',selected,c1,c2,toner,essence,serum,moist,moist2,deviceGel,usesDevice,bha,false,false,false,isMature,isHighSens,''):''}
      ${numPhases>=3?renderPhase('p3',selected,c1,c2,toner,essence,serum,moist,moist2,deviceGel,usesDevice,bha,retinalProd,aha,peel,isMature,isHighSens,''):''}
      ${numPhases>=4?renderPhase('p4',selected,c1,c2,toner,essence,serum,moist,moist2,deviceGel,usesDevice,bha,retinalProd,aha,peel,isMature,isHighSens,''):''}
    </div>`;
}
...
function renderMorningPhases(selected,toner,essence,serum,moist,spf,c1,c2,isHighSens){
  ...
  // Only use toner in Barrier Repair if it contains NO strong actives (no AHA/BHA/retinoid/etc.)
  const barrierToner = toner && isBarrierSafeProduct(toner) ? toner : null;
  const barrierHtml=`
...
function switchRoutinePhase(pid,btn){
  ...
  // searching globally would activate the wrong (hidden) panel.
  const scope=btn?btn.closest('.builder-card'):null;
  const root=scope||document;
  root.querySelectorAll('.phase-panel').forEach(p=>p.classList.remove('active'));
  root.querySelectorAll('.phase-tab').forEach(b=>b.classList.remove('active'));
  const panel=root.querySelector('#rp-'+pid)||document.getElementById('rp-'+pid);
...
function renderPhase(pid,selected,c1,c2,toner,essence,serum,moist,moist2,deviceGel,usesDevice,bha,retinal,aha,peel,isMature,isHighSens,activeClass,isOptional){
  ...
  const dayPanels=days.map(d=>{
    ...
    return `
      <div class="day-panel ${d==='Mon'?'active':''}" id="dp-${pid}-${d}">
        <div class="day-card">
          ...
          <div class="day-card-body">
            <div class="label-xs">${t('night_routine')}</div>
            ${oilCleanser&&!isRec?`<div class="routine-step">${sn()}<div class="rs-emoji">${prodEmoji(oilCleanser)}</div><div class="rs-body"><div class="rs-brand">${oilCleanser.brand}</div><div class="rs-name">${oilCleanser.name}</div><div class="rs-note">${t('step_c1_note')}</div></div></div>`:''}
            ...
            ${isRet&&retinal?`
              <div class="routine-step r-retinal">${sn('rt')}<div class="rs-emoji">${prodEmoji(moist)}</div><div class="rs-body"><div class="rs-name">${t('step_sandwich_1_name')}</div><div class="rs-note">${t('step_sandwich_1_note')}</div></div></div>
            ...
            `:moist?`<div class="routine-step ${isRec?'r-recovery':''}">${sn(isRec?'re':'n')}<div class="rs-emoji">${prodEmoji(moist)}</div><div class="rs-body"><div class="rs-brand">${moist.brand}</div><div class="rs-name">${moist.name}</div></div></div>`:''}
            ${moist2&&isRec?`<div class="routine-step r-recovery">${sn('re')}<div class="rs-emoji">${prodEmoji(moist2)}</div><div class="rs-body"><div class="rs-brand">${moist2.brand}</div><div class="rs-name">${moist2.name}</div><div class="rs-note">${t('step_overnight_note')}</div></div></div>`:''}
  ...
  return `<div class="phase-panel ${activeClass}" id="rp-${pid}"><div class="phase-hero-box ${ph.cls}"><div class="ph-tag">${tFmt('result_phase_label',{n:pid.replace('p','')})}</div><div class="ph-title">${ph.title}</div><div class="ph-desc">${ph.desc}</div><div class="ph-duration">${ph.dur}</div></div>${isOptional?`<div class="info-box amber" style="margin:10px 0 8px;display:flex;align-items:flex-start;gap:8px"><span style="font-size:1.1em;flex-shrink:0">💚</span><div><strong>${t('phase1_optional_badge')}</strong> — ${t('phase1_optional_note')}</div></div>`:''}<div class="day-nav-wrap"><div class="day-nav" id="dn-${pid}">${dayBtns}</div></div><div class="day-panels-container" id="dpc-${pid}">${dayPanels}</div></div>`;
}

/* ═══ DAY INTERACTION — EVENT DELEGATION (FIXED) ═══ */
function attachDayInteractions(){
  // Attach to whichever container is showing the routine result.
  // Builder uses #builder-content; My Routine uses #myroutine-content.
  // Both can contain interactive day buttons rendered by renderPhase().
  ['builder-content','myroutine-content'].forEach(id=>{
    const c=document.getElementById(id);
    if(!c)return;
    c.removeEventListener('click',dayClickHandler);
    c.addEventListener('click',dayClickHandler);
  });
}
...
function selectDay(pid,day,btn){
  // Scope to the routine card containing this button — see switchRoutinePhase rationale.
  const scope=btn?btn.closest('.phase-panel')||btn.closest('.builder-card'):null;
  const root=scope||document;
  const container=root.querySelector('#dpc-'+pid)||document.getElementById('dpc-'+pid);
  if(container)container.querySelectorAll('.day-panel').forEach(p=>p.classList.remove('active'));
  const nav=root.querySelector('#dn-'+pid)||document.getElementById('dn-'+pid);
  if(nav)nav.querySelectorAll('.day-btn').forEach(b=>b.classList.remove('active'));
  const panel=root.querySelector('#dp-'+pid+'-'+day)||document.getElementById('dp-'+pid+'-'+day);
  if(panel)panel.classList.add('active');
  if(btn)btn.classList.add('active');
}
...
function detectConflicts(selected){
  ...
  const hasPeel=selected.some(p=>p.category==='exfoliant'&&p.subcategory!=='chemical exfoliant');
  // Other actives
  ...
  const conflicts=[];
  // Original rules
  if(hasRetinal&&hasGlycolic)conflicts.push({combo:'Retinal + Glycolic Acid',reason:'Never same night — major barrier disruption.'});
  if(hasRetinal&&hasBHA)conflicts.push({combo:'Retinal + Salicylic Acid (BHA)',reason:'Use on separate nights only.'});
  if(hasRetinal&&hasPeel)conflicts.push({combo:'Retinal + Physical Peeling Gel',reason:'Never same night — over-exfoliation and PIH risk.'});
  if(hasGlycolic&&hasPeel)conflicts.push({combo:'AHA + Peeling Gel',reason:'Never on the same night.'});
  if(hasGlycolic&&hasBHA)conflicts.push({combo:'AHA + BHA',reason:'Too much for sensitive skin — use on separate nights.'});
  // Extended rules
  if(hasRetinol&&hasAnyAcid&&!hasRetinal)conflicts.push({combo:'Retinol + Exfoliating Acid',reason:'Never same night — causes barrier damage and irritation.'});
  if(hasBP&&hasAnyRetinoid)conflicts.push({combo:'Benzoyl Peroxide + Retinoid',reason:'BP oxidises retinoids and strips barrier — use on separate nights.'});
  if(hasBP&&hasStrongVC)conflicts.push({combo:'Benzoyl Peroxide + Vitamin C (L-Ascorbic Acid)',reason:'BP oxidises Vitamin C — use on separate AM/PM routines.'});
  if(retinoidCount>1)conflicts.push({combo:'Multiple Retinoids',reason:'Never stack retinoids — severe irritation and barrier damage risk.'});
  if(exfoliantCount>1)conflicts.push({combo:'Multiple Exfoliating Acids',reason:'Stacking acids causes over-exfoliation — use only one acid per routine.'});
  if(hasStrongVC&&hasAnyAcid)conflicts.push({combo:'Strong Vitamin C + Exfoliating Acid',reason:'High irritation risk — use Vitamin C in AM and acids in PM only.'});
  return conflicts;
}
...
function runConflictCheck(){
  ...
  if(!sel.length){r.innerHTML='<div class="notice">Select at least 2 products to check.</div>';return;}
  const conflicts=detectConflicts(sel),extras=[];
  if(sel.some(p=>!p.fragranceFree))extras.push({type:'danger',title:'Fragrance Detected',body:'One or more products contain fragrance — not recommended for sensitive skin.'});
  if(sel.some(p=>!p.eoFree))extras.push({type:'warn',title:'Essential Oil Detected',body:'One or more products contain essential oils — may irritate reactive skin.'});
  const acCount=sel.filter(p=>(p.activeIngredients||[]).some(a=>['retinal','aha','bha'].includes(a))).length;
  if(acCount>2)extras.push({type:'danger',title:'Too Many Active/Exfoliant Products',body:`${acCount} active products — max 1 per night for sensitive skin.`});
  const all=[...conflicts.map(c=>({type:'danger',title:`🚫 ${c.combo}`,body:c.reason})),...extras];
  if(!all.length){r.innerHTML=`<div class="conflict-result"><div class="conflict-head ok">✅ No Major Conflicts Found in ${sel.length} Selected Products</div><div class="conflict-body">These products are generally safe to use together. Follow phase schedules.</div></div>`;}
...
function renderPersonalizedEmergencyHTML(rd){
  ...
  const a=rd.answers||{};
  const harshActives=['retinal','retinol','aha','bha','azelaic acid','vitamin c'];
  const stopProducts=sel.filter(p=>(p.activeIngredients||[]).some(x=>harshActives.includes(x))||p.category==='exfoliant'||p.subcategory==='peeling');
  const safeProducts=sel.filter(p=>
    ...
    &&p.alcoholFree!==false
    &&p.eoFree!==false
    &&!(p.activeIngredients||[]).some(x=>harshActives.includes(x))
  );
...
function saveCurrentRoutine(){
  if(!builderState.routineData){alert('Build a routine first.');return;}
  ...
  alert('Routine saved to Glowphase! ✓');
      ...
      <div class="myr-chips">
        ${routines.map(r=>`
          <button class="myr-chip ${r.id===selectedId?'active':''}" onclick="loadRoutine('${r.id}')">
            ...
            <span class="myr-chip-meta">${r.phases||1} phase${(r.phases||1)!==1?'s':''}</span>
          </button>
        `).join('')}
      </div>
    </div>`:'';
...
function generateRecommendations(rd){
  ...
  if(wantsAcneSupport&&!selected.some(p=>(p.activeIngredients||[]).some(ai=>['bha','azelaic acid'].includes(ai)))){
    ...
    if(acne)recs.push({product:acne,reason:t('rec_reason_acne'),supports:[t('g_acne'),t('g_pih')],priority:2,caution:isSensitive?'Start 2-3× per week and patch test first':null});
  }
  ...
  if(needsAntiAging&&!selected.some(p=>(p.activeIngredients||[]).some(ai=>['peptides','pdrn','retinal','retinol'].includes(ai)))){
    ...
    if(aa){
      const usesRetinoid=(aa.activeIngredients||[]).some(ai=>['retinal','retinol'].includes(ai));
      recs.push({product:aa,reason:t('rec_reason_aging'),supports:[t('g_antiaging'),t('g_elasticity')],priority:3,caution:usesRetinoid?'Introduce slowly (2× per week). Always pair with SPF. Not during pregnancy.':null});
    }
  }
  ...
  const activeCount=selected.filter(p=>(p.activeIngredients||[]).some(ai=>['retinal','retinol','aha','bha','vitamin c'].includes(ai))).length;
  if(activeCount>=3&&!recs.some(r=>r.reason===t('rec_reason_calming'))){
    ...
    if(calmRec)recs.push({product:calmRec,reason:t('rec_reason_too_many'),supports:[t('g_calm'),t('g_barrier')],priority:1,caution:'Use on recovery nights between actives'});
  }
...
function exportRoutines(){
  const routines=getSavedRoutines();if(!routines.length){alert('No saved routines to export.');return;}
...
function importRoutines(e){
  ...
  reader.onload=ev=>{
    try{
      ...
      alert(`Imported ${newOnes.length} routine${newOnes.length!==1?'s':''}. ${data.length-newOnes.length} already existed.`);
      renderMyRoutines();
