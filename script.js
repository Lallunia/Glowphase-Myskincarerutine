...
function _renderDscQuestions(){
  ...
  function optRow(q,key,label){
    ...
    return `<div class="wsc-option ${sel}" onclick="dscSelectAnswer('${q}','${key}')"><div class="wsc-option-dot"></div>${label}</div>`;
  }
  document.getElementById('dsc-sheet-box').innerHTML=`
    ...
    <div class="wsc-question">
      <div class="wsc-q-label">${t('dsc_q1_label')}</div>
    ...
    <div class="wsc-question">
      <div class="wsc-q-label">${t('dsc_q2_label')}</div>
      <div class="wsc-options">
...
function renderRoutineResultBody(rd, _recordUsage){
...
const _rpIsModerate=_rpA.complexity===t('o_moderate_r');
  ...
  const _p3FocusTabs=pid==='p3'?`<div class="p4focus-wrap">
    ...
    <div class="p4focus-note">${t('p4focus_switch_note')}</div>
    ${(_p3Focus==='renew'&&_p3Fragile)?`<div class="info-box amber" style="margin-top:10px;display:flex;align-items:flex-start;gap:8px"><span style="font-size:1.1em;flex-shrink:0">⚠️</span><div><strong>${t('p3focus_retinal_warn_title')}</strong><div style="font-size:.8rem;color:#7a6878;margin-top:3px;line-height:1.5">${t('p3focus_retinal_warn_body')}</div></div></div>`:''}
  </div>`:'';
