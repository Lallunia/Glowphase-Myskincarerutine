...
function hasExfoliantAcid(p) {
  ...
  const ing = (p.ingredients || '').toLowerCase();
  return /\b(glycolic acid|lactic acid|mandelic acid|salicylic acid|beta hydroxy|poly hydroxy|lactobionic|gluconolactone|capryloyl salicylic|azelaic acid)\b/.test(ing);
}
...
// P4 has longer thresholds because it's a long-term maintenance phase
function _getPhaseSubStateLevel(pid,days,focus){
  // Anti-aging uses longer thresholds since retinoids take 8-12 weeks
  const thresholds={p1:[7,21],p2:[7,21],p3:[7,21],p4:[14,35]};
  if(pid==='p4'&&focus==='aging')return days<=21?1:days<=56?2:3;
  const th=thresholds[pid]||[7,21];
  return days<=th[0]?1:days<=th[1]?2:3;
}
function _getPhaseSubState(pid,days,focus){
  const lv=_getPhaseSubStateLevel(pid,days,focus);
  if(pid==='p4'&&focus&&focus!=='aging'||pid==='p4'&&focus==='aging'){
...
// Injects a small sub-state badge chip below the phase label in a routine card.
function _injectPhaseSubStateBadge(card,pid){
  const old=card.querySelector('.js-sub-badge');if(old)old.remove();
  const r=getSavedRoutines().find(function(x){return x.id===(card.dataset.cardId||'').replace('gc-','');});
  if(!r)return;
  const _start=r.phaseStartedAt||r.createdAt;if(!_start)return;
  const days=Math.max(1,Math.floor((Date.now()-new Date(_start).getTime())/(1000*60*60*24))+1);
  const focus=(pid==='p4')?(r.p4Focus||'aging'):undefined;
  const label=_getPhaseSubState(pid,days,focus);
  const tip=_getPhaseSubStateTip(pid,days,focus);
  const badge=document.createElement('div');
  badge.className='js-sub-badge';
  badge.title=tip||'';
  badge.textContent=label;
  const jsStrip=card.querySelector('.js-strip');
  if(jsStrip)jsStrip.insertAdjacentElement('afterend',badge);
}
...
const sleepingPack=selected.find(p=>normalizedCategory(p)==='sleeping mask'||p.subcategory==='sleeping mask'||p.subcategory==='sleeping pack')||null;
  ...
  const aha=(()=>{
    // Include exfoliating toner pads (BHA/AHA/PHA pads) — they belong in the exfoliant slot, not toner slot
    const cands=selected.filter(p=>(hasExfoliantAcid(p)||_isExfPad(p))&&p.subcategory!=='spot treatment'&&p!==moist&&!_isTonerSlotPad(p));
    if(!cands.length)return null;
    const scored=cands.map(p=>({p,s:scoreProductForUser(p,a)})).sort((a,b)=>b.s-a.s);
    const topScore=scored[0].s;
    const tier=scored.filter(x=>x.s>=topScore-2);
...
const _rpIsModerate=_rpA.complexity===t('o_moderate_r');
  ...
  const dayPanels=[selectedDay||'Mon'].map(d=>{
    ...
    const _showGelCream=!!(_dayGelCream&&!_rpIsSimple&&!isBarrierRecovery);
