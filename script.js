...
function renderRoutineResultBody(rd){
  ...
  const allToners=selected.filter(p=>normalizedCategory(p)==='toner'||_isTonerSlotPad(p));
...
const _rpIsModerate=_rpA.complexity===t('o_moderate_r');
  ...
  const dayPanels=[selectedDay||'Mon'].map(d=>{
    ...
    if(_rpIsModerate&&(_showEssence||_showEye||_showMistStep)){
      ...
      if(isPeel&&peel&&!isBarrierRecovery&&!_rpDamagedBarrier)_base++;                            // peeling gel
      ...
      _base++;
      if(isPeel&&peel&&!isBarrierRecovery&&!_rpDamagedBarrier)_base++;
      if(dayToner)_base++;
